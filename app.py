...
def start_batch(uploaded, upload_mode, options: BatchOptions, background: bool) -> None:
    """Validate uploads and submit a batch job. Shared by both views.

    Keeps behavior identical to the original inline submit body: ZIP extraction
    or per-file validation, the batch-limit check, ``manager.submit`` and the
    ``job_id`` / ``validation_messages`` session state plus the success message.
    """
...
# View switch (outside the fragment so switching re-runs the whole app). A
# horizontal radio keeps the two modes obvious to non-technical users and is
# reliably driveable by Streamlit's AppTest harness.
view = st.radio(
    "View",
    ["Simple", "Advanced"],
    horizontal=True,
    key="view",
    help="Simple shows plain results; Advanced exposes every detail and control.",
)
view = view or "Simple"
...
# Shared uploader (both views).
if view == "Simple":
    st.caption("Upload parcel photos and we'll read the tracking number, weight and size from each one.")
...
else:  # Simple view: one primary button, never enables vision.
    if st.button("Process images", type="primary", disabled=not uploaded):
        start_batch(
            uploaded,
            upload_mode,
            BatchOptions(
                local_only=True,
                enable_vision=False,
                generate_annotations=config.generate_annotations,
...
def job_monitor() -> None:
    ...
    view = st.session_state.get("view", "Simple")
    if view == "Advanced":
    ...
    # Simple view: friendly, minimal — no JSON, enums, confidences or filters.
    if not terminal:
        st.progress(min(1.0, job.progress_percentage / 100))
        st.caption(f"Reading photo {job.processed_images} of {job.total_images}…")
        if st.button("Stop"):
            manager.cancel(job_id)
            st.info("Stopping after the current photo finishes.")
    if job.stage in {JobStage.FAILED, JobStage.CANCELLED}:
        st.error(job.error_message or f"Job ended with status {job.stage.value}")
    if can_resume:
        if st.button("Resume", type="primary"):
            try:
                manager.resume(job_id, background=True)
                st.success("Resuming the unfinished photos.")
            except (ValueError, RuntimeError) as exc:
                st.error(str(exc))
    ...
    if not results:
        if not terminal:
            st.info("Working on your photos…")
        else:
            st.info("No results yet.")
        return

    simple_frame = simple_dataframe(results)
    photos_read = len(results)
    ready_to_use = sum(1 for result in results if not result.requires_review)
    need_a_check = sum(1 for result in results if result.requires_review)
    metric_columns = st.columns(3)
    metric_columns[0].metric("Photos read", photos_read)
    metric_columns[1].metric("Ready to use", ready_to_use)
    metric_columns[2].metric("Need a check", need_a_check)
    ...
    st.download_button(
        ...
        data=simple_csv_bytes(results),
        file_name="parcel_results.csv",
        mime="text/csv",
    )

    with st.expander("Full data files (advanced)"):
        if job.report_path and Path(job.report_path).exists():
            st.download_button(
                "Download full CSV",
                data=Path(job.report_path).read_bytes(),
    ...
    filenames = [result.original_filename for result in results]
    selected_filename = st.selectbox("See a parcel", filenames)
    selected = next(result for result in results if result.original_filename == selected_filename)
    input_path = manager.database.result_input_path(job_id, selected.image_id)
    if input_path and Path(input_path).exists():
        st.image(input_path, caption=selected.original_filename, width="stretch")

    descriptor = outcome(selected)
    headline = f'{descriptor["emoji"]} {descriptor["label"]}'
    if descriptor["tone"] == "good":
        st.success(headline)
    elif descriptor["tone"] == "warn":
        st.warning(headline)
    else:
        st.error(headline)

    st.write(f"**Tracking number:** {plain_awb(selected)}")
    st.write(f"**Weight:** {plain_weight(selected)}")
    st.write(f"**Size:** {plain_size(selected)}")
    st.write(f"**Location:** {plain_location(selected)}")
    reasons = plain_reasons(selected)
    if reasons:
        st.caption("What to check")
